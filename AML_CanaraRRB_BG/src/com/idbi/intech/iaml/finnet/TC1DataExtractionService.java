...
public class TC1DataExtractionService {
	...
	public static List<String> getReportData(String reportType, String fromDate, String toDate)
	{
		...
		try
		{
			...
			{
				sql = "select tran_id||'~'||to_char(tran_date,'"+patternfordb2+"')||'~'||CUST_ID||'~'||ACCOUNT_NO||'~'||CRE_DEB_FLG||'~'||TRAN_AMT||'~'||SOL_ID||'~'||decode(SUBSTR(tran_particular, 1, 7),'CDS/CRT',lpad(regexp_substr(tran_particular,'[^ /]+',1,4),4,'0'),'ATM WDR',regexp_substr(tran_particular,'[^ ]+',1,3),'NA')||'~'||SUBSTR(tran_particular, 1, 7)||'~'||REPORT_REF_NUM from ntr_details_finnet where TRAN_CHANNEL = 'ATM' and tran_date between '"+fromDate+"' and '"+toDate+"' ";
			}
			else if(reportType.equals("CTR"))
			{
				sql = "select tran_id||'~'||to_char(tran_date,'"+patternfordb2+"')||'~'||TRAN_CUST_ID||'~'||ACCOUNT_NO||'~'||CRE_DEB_FLG||'~'||TRAN_AMT||'~'||SOL_ID||'~'||decode(SUBSTR(tran_particular, 1, 7),'CDS/CRT',lpad(regexp_substr(tran_particular,'[^ /]+',1,4),4,'0'),'ATM WDR',regexp_substr(tran_particular,'[^ ]+',1,3),'NA')||'~'||SUBSTR(tran_particular, 1, 7)||'~'||REPORT_REF_NUM from ctr_details_finnet where TRAN_CHANNEL = 'ATM' and tran_date between '"+fromDate+"' and '"+toDate+"' ";
			}
			else
			{
	...
	public static void main(String[] args) throws IOException, SQLException {
		...
		amlProp.load(is);
		is.close();
		String threadSize = amlProp.getProperty("THREAD_SIZE");

		makeConnection();
		
		RequestDataBean reqObj = ConstantFunctions.getRequestData("TC1");
		...
		List<String> reportList = getReportData(reportType, fromDate, toDate);
	    ...
	    for (int start = 0; start < reportList.size(); start += size) {
	        ...
	        new Thread(new TC1Thread(sublist, requestId, batchNo, fromDate, toDate, reportType,tid)).start();
	    }
