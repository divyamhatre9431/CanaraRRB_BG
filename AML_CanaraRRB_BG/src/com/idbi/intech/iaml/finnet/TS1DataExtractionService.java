...
public class TS1DataExtractionService {
		...
		try {
			if (reportType.equals("NTR")) {
				sql = "select to_char(tran_date,'" + patternfordb2
						+ "')||'~'||decode(TRAN_CHANNEL,'NEFT',SUBSTR(TRAN_RMKS,1,16),'RTGS',SUBSTR(TRAN_PARTICULAR,7,22),'NA')||'~'||decode(CRE_DEB_FLG,'C','I','D','O',CRE_DEB_FLG)||'~'||decode(TRAN_CHANNEL,'RTGS','NRTGS',TRAN_CHANNEL)||'~'||REPORT_REF_NUM||'~'||cust_id  from ntr_details_finnet where TRAN_CHANNEL in ('NEFT','RTGS') and tran_date between '"
						+ fromDate + "' and '" + toDate + "' ";
			} else if (reportType.equals("CTR")) {
				sql = "select to_char(tran_date,'" + patternfordb2
						+ "')||'~'||decode(TRAN_CHANNEL,'NEFT',SUBSTR(TRAN_RMKS,1,16),'RTGS',SUBSTR(TRAN_PARTICULAR,7,22),'NA')||'~'||decode(CRE_DEB_FLG,'C','I','D','O',CRE_DEB_FLG)||'~'||decode(TRAN_CHANNEL,'RTGS','NRTGS',TRAN_CHANNEL)||'~'||REPORT_REF_NUM from ctr_details_finnet where TRAN_CHANNEL in ('NEFT','RTGS') and tran_date between '"
						+ fromDate + "' and '" + toDate + "' ";
			} else {
	...
	public static void main(String[] args) throws IOException, SQLException {
		...
		amlProp.load(is);
		is.close();
		String threadSize = amlProp.getProperty("TS1_THREAD_SIZE");

		makeConnection();

		RequestDataBean reqObj = ConstantFunctions.getRequestData("TS1");
	    ...
	    for (int start = 0; start < reportList.size(); start += size) {
	        ...
	        new Thread(new TS1Thread(sublist, requestId, batchNo, fromDate, toDate, reportType,tid)).start();
	    }
