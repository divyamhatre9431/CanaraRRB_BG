...
public class GenerateStrAPR implements Runnable{
	...
	public GenerateStrAPR(String strNo) {
		this.strNo = strNo;
	}
	...
	String InsertApr=	"insert into REG_STR_ACCOUNT_PERSON_RELTN(account_number,related_person_type,name_of_noncustomer,unique_ref_no,ind_nid_status,"
		...
		try (PreparedStatement pstmtIsertIntoAml = connection.prepareStatement(InsertApr);) {
			...
			if (cnt > 0) {
				...
				String procedure = "update_reg_apr_str_data";
				System.out.println(
						"Started execution for Cleaning StrApr: " + procedure + "For the Request id" + vRequestId);
				stmt = connection.prepareCall("call update_reg_apr_str_data('" + vRequestId + "')");
				stmt.execute();
