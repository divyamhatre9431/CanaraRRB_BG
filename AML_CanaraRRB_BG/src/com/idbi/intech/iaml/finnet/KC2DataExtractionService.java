...
public class KC2DataExtractionService {
		...
		try {
			if (reportType.equals("NTR")) {
				sql = "select distinct cust_id||'-'||REPORT_REF_NUM from ntr_details_finnet where tran_date between '" + fromDate
						+ "' and '" + toDate + "' and cust_type='N' ";
			} else if (reportType.equals("CTR")) {
				sql = "select distinct tran_cust_id||'-'||REPORT_REF_NUM from ctr_details_finnet where tran_date between '" + fromDate
						+ "' and '" + toDate + "' and cust_type='N' ";
			} else {
				sql = "select distinct cust_id||'-'||'NA' acid from aml_str_req_acct where req_no='" + strNo
						+ "' and get_cust_type(cust_id)='N' ";
			}
	...
	public static void main(String[] args) throws IOException, SQLException {
		...
		amlProp.load(is);
		is.close();
		String threadSize = amlProp.getProperty("KC2_THREAD_SIZE");

		makeConnection();

		RequestDataBean reqObj = ConstantFunctions.getRequestData("KC2");
	    ...
	    for (int start = 0; start < reportList.size(); start += size) {
	        ...
	        new Thread(new KC2Thread(sublist, requestId, batchNo, reportType,tid)).start();
	    }
	}
