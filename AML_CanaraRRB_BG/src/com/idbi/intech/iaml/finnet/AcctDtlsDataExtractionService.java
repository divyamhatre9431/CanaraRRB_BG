...
public class AcctDtlsDataExtractionService {
	...
	public static List<String> getReportData(String reportType, String fromDate, String toDate, String strNo) {
		...
		try {
			...
			if (reportType.equals("NTR")) {
				sql = "select distinct cust_id||'-'||account_no||'-'||REPORT_REF_NUM from ntr_details_finnet where tran_date between '"
						+ fromDate + "' and '" + toDate + "' ";
			} else if (reportType.equals("CTR")) {
				sql = "select distinct tran_cust_id||'-'||account_no||'-'||REPORT_REF_NUM from ctr_details_finnet where tran_date between '"
						+ fromDate + "' and '" + toDate + "' ";
			} else {
				sql = "select distinct cust_id||'-'||ACCT_NO from aml_str_req_acct where req_no='" + strNo + "' ";
			}
	...
	public static void main(String[] args) throws IOException, SQLException {
		...
		amlProp.load(is);
		is.close();
		String threadSize = amlProp.getProperty("ACCT_DTLS_THREAD_SIZE");

		makeConnection();

		RequestDataBean reqObj = ConstantFunctions.getRequestData("AD");
	    ...
	    for (int start = 0; start < reportList.size(); start += size) {
	        ...
	        new Thread(new AcctDtlsThread(sublist, requestId, batchNo, fromDate, toDate,reportType, tid)).start();
	    }
	}
