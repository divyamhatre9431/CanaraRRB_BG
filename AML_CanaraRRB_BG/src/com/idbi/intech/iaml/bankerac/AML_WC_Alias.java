...
public class AML_WC_Alias {
	...
	public void readXML(String file, Connection connection)
		...
		try {
			pstmt = connection
					.prepareStatement("insert into aml_wl_alias values (?,?)");
			...
			while (eventReader.hasNext()) {
				...
				if (event.isStartElement()) {
					StartElement startElement = event.asStartElement();
					if (startElement.getName().getLocalPart().equals("alias")) {
						alias = eventReader.getElementText();

						if (!alias.equals("")) {
							pstmt.setString(1, id);
							pstmt.setString(2, alias.toUpperCase());
