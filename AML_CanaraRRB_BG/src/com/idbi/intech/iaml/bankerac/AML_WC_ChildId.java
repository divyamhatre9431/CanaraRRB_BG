...
public class AML_WC_ChildId {
	...
	public void readXML(String file, Connection connection)
		...
		try {
			pstmt = connection
					.prepareStatement("insert into aml_wl_ChildId values (?,?)");
			...
			while (eventReader.hasNext()) {
				...
				if (event.isStartElement()) {
					...
					if (event.asStartElement().getName().getLocalPart()
						...
						while (eventReader.peek() != null
							...
							child = event.asCharacters().getData() == null ? ""
				...
				if (event.isEndElement()) {
					...
					if (endElement.getName().getLocalPart() == "childId") {
						if (!child.equals("")) {
